package atm;

import javax.swing.*;
import java.awt.*;
import java.awt.event.*;
import java.sql.*;

public class Signup extends JFrame implements ActionListener {

    JTextField nameText, fnameText, dobText, emailText;
    JTextField addressText, cityText, pinText, stateText;
    JButton next;

    JRadioButton male, female;
    ButtonGroup genderGroup;

    JRadioButton married, unmarried, other;
    ButtonGroup maritalGroup;

    JLabel heading;
    long applicationNumber;

    Signup() {

        setTitle("New Account Application Form");
        setLayout(null);

        // 🔥 Heading with Application Number
        heading = new JLabel("Application Form No: ");
        heading.setFont(new Font("Arial", Font.BOLD, 26));
        heading.setBounds(120, 10, 400, 30);
        add(heading);

        // ✅ Generate temporary application number (6 digit random)
        applicationNumber = generateApplicationNumber();
        heading.setText("Application Form No: " + applicationNumber);

        // Name
        JLabel name = new JLabel("Name:");
        name.setBounds(60, 80, 120, 25);
        add(name);

        nameText = new JTextField();
        nameText.setBounds(200, 80, 250, 25);
        add(nameText);

        // Father's Name
        JLabel fname = new JLabel("Father's Name:");
        fname.setBounds(60, 120, 120, 25);
        add(fname);

        fnameText = new JTextField();
        fnameText.setBounds(200, 120, 250, 25);
        add(fnameText);

        // DOB
        JLabel dob = new JLabel("Date of Birth:");
        dob.setBounds(60, 160, 120, 25);
        add(dob);

        dobText = new JTextField();
        dobText.setBounds(200, 160, 250, 25);
        add(dobText);

        // Gender
        JLabel gender = new JLabel("Gender:");
        gender.setBounds(60, 200, 120, 25);
        add(gender);

        male = new JRadioButton("Male");
        male.setBounds(200, 200, 80, 25);
        female = new JRadioButton("Female");
        female.setBounds(300, 200, 80, 25);

        genderGroup = new ButtonGroup();
        genderGroup.add(male);
        genderGroup.add(female);

        add(male);
        add(female);

        // Email
        JLabel email = new JLabel("Email:");
        email.setBounds(60, 240, 120, 25);
        add(email);

        emailText = new JTextField();
        emailText.setBounds(200, 240, 250, 25);
        add(emailText);

        // Marital Status
        JLabel marital = new JLabel("Marital Status:");
        marital.setBounds(60, 280, 120, 25);
        add(marital);

        married = new JRadioButton("Married");
        married.setBounds(200, 280, 90, 25);
        unmarried = new JRadioButton("Unmarried");
        unmarried.setBounds(300, 280, 100, 25);
        other = new JRadioButton("Other");
        other.setBounds(410, 280, 80, 25);

        maritalGroup = new ButtonGroup();
        maritalGroup.add(married);
        maritalGroup.add(unmarried);
        maritalGroup.add(other);

        add(married);
        add(unmarried);
        add(other);

        // Address
        JLabel address = new JLabel("Address:");
        address.setBounds(60, 320, 120, 25);
        add(address);

        addressText = new JTextField();
        addressText.setBounds(200, 320, 250, 25);
        add(addressText);

        // City
        JLabel city = new JLabel("City:");
        city.setBounds(60, 360, 120, 25);
        add(city);

        cityText = new JTextField();
        cityText.setBounds(200, 360, 250, 25);
        add(cityText);

        // PIN
        JLabel pin = new JLabel("PIN Code:");
        pin.setBounds(60, 400, 120, 25);
        add(pin);

        pinText = new JTextField();
        pinText.setBounds(200, 400, 250, 25);
        add(pinText);

        // State
        JLabel state = new JLabel("State:");
        state.setBounds(60, 440, 120, 25);
        add(state);

        stateText = new JTextField();
        stateText.setBounds(200, 440, 250, 25);
        add(stateText);

        // NEXT Button
        next = new JButton("NEXT");
        next.setBounds(220, 480, 120, 30);
        next.addActionListener(this);
        add(next);

        setSize(600, 600);
        setLocation(400, 50);
        setVisible(true);
    }

    // 🔥 Generate random temporary application number
    private long generateApplicationNumber() {
        long num = 100000 + (long)(Math.random() * 900000); // 6 digit number
        return num;
    }

    public void actionPerformed(ActionEvent ae) {

        try {

            String name = nameText.getText();
            String fatherName = fnameText.getText();
            String dob = dobText.getText();
            String email = emailText.getText();
            String address = addressText.getText();
            String city = cityText.getText();
            String pin = pinText.getText();
            String state = stateText.getText();

            String gender = male.isSelected() ? "Male" : "Female";

            String marital = null;
            if (married.isSelected()) marital = "Married";
            else if (unmarried.isSelected()) marital = "Unmarried";
            else marital = "Other";

            DBConnection db = new DBConnection();

            String query = "INSERT INTO signup(application_no, name, father_name, dob, gender, email, marital_status, address, city, pin_code, state) VALUES(?,?,?,?,?,?,?,?,?,?,?)";

            PreparedStatement pst = db.con.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);

            pst.setLong(1, applicationNumber); // 🔥 Use our application number
            pst.setString(2, name);
            pst.setString(3, fatherName);
            pst.setString(4, dob);
            pst.setString(5, gender);
            pst.setString(6, email);
            pst.setString(7, marital);
            pst.setString(8, address);
            pst.setString(9, city);
            pst.setString(10, pin);
            pst.setString(11, state);

            pst.executeUpdate();

            JOptionPane.showMessageDialog(null,
                    "Signup Successful!\nApplication Number: " + applicationNumber);

            setVisible(false);
            new Signup2(applicationNumber);

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static void main(String[] args) {
        new Signup();
    }
}